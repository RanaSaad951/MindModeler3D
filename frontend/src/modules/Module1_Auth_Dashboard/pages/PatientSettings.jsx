...
const LockedField = ({ label, value, icon: Icon }) => (
  <div>
    ...
    <div className="relative">
      ...
      <div className="w-full pl-11 pr-4 py-3.5 rounded-full text-sm bg-white/[0.03] border border-white/10 text-slate-500 cursor-not-allowed select-none truncate">
        {value || '—'}
      </div>
    </div>
  </div>
...
const PatientSettings = () => {
  ...
  const [form, setForm] = useState({ age: '', contactNumber: '', city: '' });
  ...
  /* ── Populate form from mongoProfile on mount ─────────────── */
  useEffect(() => {
    if (mongoProfile) {
      setForm({
        age:           mongoProfile.age?.toString()   || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.age && (isNaN(form.age) || Number(form.age) < 1 || Number(form.age) > 150)) {
    ...
    try {
      ...
      if (!res.ok) {
        setFormError(data.message || 'Failed to save changes.');
        setLoading(false);
        setUploadProgress('');
        return;
      }
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-8 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-md animate-slide-up py-6">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-cyan-400 tracking-tight">Profile Settings</h1>
          ...
          <div className="bg-black/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 space-y-6"
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="flex flex-col items-center gap-3">
                <div
                  className="relative w-28 h-28 rounded-full border-2 border-dashed border-cyan-500/40 bg-white/[0.04]
                    flex items-center justify-center cursor-pointer group hover:border-cyan-400/70 transition-all duration-300 overflow-hidden"
                  onClick={() => fileInputRef.current?.click()}
                >
                  {picPreview ? (
                    <img src={picPreview} alt="Profile" className="w-full h-full object-cover rounded-full" />
                  ) : (
                ...
                <input ref={fileInputRef} type="file" accept="image/*" className="hidden"
                  onChange={handlePicChange} id="settings-pic-input" />
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-4 p-4 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
                  <LockedField label="Patient ID" value={mongoProfile?.uniquePatientId} icon={() => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>} />
                  <LockedField label="CNIC Number" value={mongoProfile?.cnic} icon={() => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0" /></svg>} />
                  <LockedField label="Full Name" value={mongoProfile?.name} icon={() => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>} />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-4">
                  <ActiveField
                    label="Age" id="settings-age" name="age" type="number"
                    placeholder="e.g. 28" min="1" max="150"
                    value={form.age} onChange={handleChange}
                    icon={IconCalendar}
                  />
                  <ActiveField
                    label="Contact Number" id="settings-contact" name="contactNumber" type="tel"
                    placeholder="+92-3XX-XXXXXXX"
                    value={form.contactNumber} onChange={handleChange}
                    icon={IconPhone}
                  />
                  <ActiveField
                    label="City" id="settings-city" name="city" type="text"
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button type="button"
                  ...
                  className="flex-1 py-3.5 rounded-full text-sm font-semibold text-slate-400
